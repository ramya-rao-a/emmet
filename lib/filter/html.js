...
define(function(require, exports, module) {
	...
	function makeAttributesString(node, profile) {
		...
		var setTabStopAtEmptyAttr = cursor === '${0}';
		var maxTabStop = -1;

		if (setTabStopAtEmptyAttr) {			
			node.attributeList().forEach(function(attr) {
				if (!attr.value) {
					return;
				}
				var matches = attr.value.match(/\$\{(\d+)\}/);
				if (!matches){
					matches = attr.value.match(/\$\{(\d+):[^\}]+\}/);
				}
				if (matches) {					
					if (matches[1] > maxTabStop) {
						maxTabStop = matches[1];
					}
				}
			});			
		}
		...
		return node.attributeList().map(function(a) {
			...
			attrValue = attrValue || (setTabStopAtEmptyAttr ? `\$\{${++maxTabStop}\}` : cursor);
			return ' ' + attrName + '=' + attrQuote + attrValue + attrQuote;
		}).join('');
	}
